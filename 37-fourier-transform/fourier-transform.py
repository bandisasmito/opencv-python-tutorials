...
def fourierTransform(): 
    '''
        FFT - gives you the freq, with high in the middle and low on the outside 
        FFTshift - give you the freq, low in the middle, and high on the outside 
    '''
    root = os.getcwd() 
    ...
    # note that after applying the mask, only a small portion of the image in freq domain is needed to describe the image! good from image compression 
    r,c = img.shape
    ...
    # laplacian - high pass filter, white outside
    # gaussian - low pass filter, white inside 
